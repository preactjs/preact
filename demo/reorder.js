...
export default class Reorder extends Component {
	...
	renderItem = item => <li key={this.state.useKeys ? item.key : null}>{item.label}</li>;
		...
		return (
			<div class="reorder-demo">
				<header>
					...
					<label>
						<input type="checkbox" onClick={this.toggleKeys} checked={useKeys} /> use keys?
					</label>
					<label>
						<input type="number" step="1" min="1" style={{ width: '3em' }} onInput={this.setCount} value={count} />{' '}
						count
					</label>
				</header>
