...
describe('render()', () => {
	...
	it('should apply style as String', () => {
		...
		expect(scratch.childNodes[0]).to.have.deep
	...
	it('should add and remove event handlers', () => {
		let click = sinon.spy(), mousedown = sinon.spy();
		...
		expect(proto.addEventListener).to.have.been.calledTwice.and.to.have.been
			.calledWith('click')
		...
		expect(proto.removeEventListener).to.have.been.calledOnce.and.calledWith(
			'mousedown'
		);

		fireEvent(scratch.childNodes[0], 'click');
		expect(click).to.have.been.calledOnce.and.to.have.been.calledWith(2);
	...
	it('should use capturing for event props ending with *Capture', () => {
		let click = sinon.spy(), focus = sinon.spy();
	...
	it('should serialize style objects', () => {
		...
		expect(root).to.have.deep
			.property('style.cssText')
			.that.equals('color: rgb(0, 255, 255);');

		root = render(<div style="display: inline;">test</div>, scratch, root);

		expect(root).to.have.deep
			.property('style.cssText')
			.that.equals('display: inline;');
		...
		expect(root).to.have.deep
			.property('style.cssText')
			.that.equals('background-color: rgb(0, 255, 255);');
	});
	...
	it('should reconcile mutated DOM attributes', () => {
		let check = p =>
			render(<input type="checkbox" checked={p} />, scratch, scratch.lastChild),
	...
	it('should ignore props.children if children are manually specified', () => {
		expect(<div a children={['a', 'b']}>c</div>).to.eql(<div a>c</div>);
	});
	...
	it('should skip non-preact elements', () => {
		...
		expect(scratch.innerHTML, 'forceUpdate').to.equal(
		...
		expect(scratch.innerHTML, 'forceUpdate alt').to.equal(
		...
		expect(
			scratch.firstChild.children,
			'root re-render changed'
		).to.have.length(3);
