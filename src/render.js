...
export function render(vnode, parent) {
	...
	if (oldTree) diff(oldTree._el, parent, parent._previousVTree = coerceToVNode(vnode), oldTree, EMPTY_OBJ, false, true, null, 1, [] );
	else hydrate(vnode, parent);
}

export function hydrate(vnode, parent) {
	parent._previousVTree = vnode = coerceToVNode(vnode);
	diffChildren(parent, [vnode], EMPTY_ARR, EMPTY_OBJ, false, EMPTY_ARR.slice.call(parent.childNodes), 1, []);
}
