		...
		if (childVNode!=null) {
			...
			if (newDom!=null) {
				...
				if (nextDom === undefined) {
					if (oldVNodeIndex == null) { // if (oldVNode == EMPTY_OBJ) {
						// console.log(newDom.nextSibling)

						if (newDom != oldDom) {
							console.log('No oldVNode was found and newDom != oldDom. This is unexpected.');
						}
					}
					else if (newDom !== oldVNode._dom) {
						console.log("oldVNode is not EMPTY_OBJ and newDom !== oldVNode._dom. This is unexpected.");
					}
					else {
						let nextVNode = null;
